...
def main():
    ...
    ap.add_argument("--centroids", required=False, default="data/index/video_centroids.npy")
    ap.add_argument("--out-dir", required=False, default="data/domain")
    args=ap.parse_args()
    out=Path(args.out_dir); out.mkdir(parents=True, exist_ok=True)

    C_path=Path(args.centroids)
    if not C_path.exists():
        print("centroids not found, skipping.")
        return
    C=np.load(C_path).astype("float32")
    # Save renormalized copy and covariance for debugging
