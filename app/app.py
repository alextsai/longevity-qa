...
Experts-first RAG with domain routing, robust quoting, trusted-web support, and streaming output.

Key behaviors
- Stage A routes 8–12 videos using domain model + semantic + keyword + recency.
- Stage B searches chunks from ≥4 videos, applies MMR, and keeps only quotes relevant to the question.
- Quotes include timestamped YouTube links. Trusted-web snippets are added as supporting evidence with links.
- Sources render in an auto-collapsed section. Page title: "Health | Nutrition Q&A".
- Admin tools: verify artifacts, rebuild/repair, creator inventory, and unique-user count persisted on disk.

Paths
- DATA_DIR default: /var/data/data  (Railway volume). Repo fallbacks auto-copy into this volume on boot.
- Domain artifacts expected at: data/domain/{domain_model.joblib, domain_probs.json|yaml}.

Env
- OPENAI_API_KEY must be set.
- WEB_FALLBACK=true enables web-only answers when video evidence is missing.
"""
...
import sys, json, yaml, pickle, time, re, math, collections, hashlib, uuid
...
# Mounted volume for production (Railway). Repo fallback auto-copies into this.
...
DOMAIN_MODEL_FILE = DOMAIN_DIR / "domain_model.joblib"
DOMAIN_PROBS_JSON = DOMAIN_DIR / "domain_probs.json"
DOMAIN_PROBS_YAML = DOMAIN_DIR / "domain_probs.yaml"

# Metrics
METRICS_DIR   = DATA_ROOT / "data/metrics"
USERS_JSON    = METRICS_DIR / "users.json"
...
def _quote_is_readable(text:str)->bool:
    t=_normalize_text(text)
    if len(t) < 40: return False
    return any(x in t for x in [". ","; ",": ","? ","! "])

# ------------------------ Bootstrap domain artifacts --------
def _ensure_domain_artifacts():
    """Copy repo /data/domain/* into mounted volume if the volume is missing them."""
...
def _get_or_create_user_id()->str:
    # Persist a uuid in query params so it survives reloads; record to disk.
    qp = st.query_params
    uid = qp.get("uid")
    if not uid:
        uid = str(uuid.uuid4())
        st.query_params.update({"uid": uid, **{k:v for k,v in qp.items() if k!="uid"}})
    METRICS_DIR.mkdir(parents=True, exist_ok=True)
    try:
        data = json.loads(USERS_JSON.read_text(encoding="utf-8")) if USERS_JSON.exists() else {}
    except Exception:
        data = {}
    if uid not in data:
        data[uid] = {"first_seen": time.time()}
        USERS_JSON.write_text(json.dumps(data, indent=2), encoding="utf-8")
    return uid

def _unique_user_count()->int:
    try:
        data = json.loads(USERS_JSON.read_text(encoding="utf-8")) if USERS_JSON.exists() else {}
        return len(data)
    except Exception:
...
def load_domain_model():
    """Return (model, per_video_probs, domain_set)."""
    if not DOMAIN_MODEL_FILE.exists() or joblib is None:
        return None, {}, set()
    try:
        model = joblib.load(DOMAIN_MODEL_FILE)
    except Exception:
        model = None
...
def _pseudo_keywords_if_missing(vid:str, summaries:dict, vm:dict)->str:
    # If no bullets, synthesize lightweight pseudo-keywords from title + channel
    info = vm.get(vid,{})
    title = info.get("title","")
    channel = info.get("channel","") or info.get("author","")
    fallback = f"{title}. {channel}"
    return " ".join(fallback.split())[:400]

def route_videos_by_summary_and_domain(
    query: str, qv: np.ndarray,
    summaries: dict, vm: dict,
    C: np.ndarray | None, vids: list[str] | None,
    allowed_vids: set[str],
    per_video_domain_probs: dict, domain_top: list[str],
    topK_min:int=8, topK_max:int=12,
    recency_weight: float=0.20, half_life_days: float=270,
    min_kw_overlap:int=1
) -> list[str]:
    """
    Hybrid routing with guardrails.
    Score = 0.45*domain + 0.35*centroid/summary + 0.20*recency
    Ensures at least topK_min and at most topK_max videos when possible.
    """
    ...
    def _domain_score(vid:str)->float:
        if not per_video_domain_probs or not domain_top: return 0.0
    ...
    for v in universe:
        ...
        if not pseudo:
            pseudo = _pseudo_keywords_if_missing(v, summaries, vm)

        kw, overlap = _kw_score(pseudo, query, idf)
        if overlap < min_kw_overlap and pseudo:  # keyword guard
            continue
        ...
        dom = _domain_score(v)
        sem = 0.7*cs + 0.3*kw
        score = 0.45*dom + 0.35*sem + 0.20*rec
        scored.append((v, score))

    scored.sort(key=lambda x:-x[1])
    if not scored: return []

    top = [v for v,_ in scored[:topK_max]]
    if len(top) < topK_min:
        extra = [v for v,_ in scored[topK_max:topK_min] ]
        top.extend(extra)
    return list(dict.fromkeys(top))[:topK_max]
...
def stageB_search_chunks(
    ...
    initial_k:int, final_k:int,
    min_distinct_videos:int,
    per_video_cap:int,
    ...
    Keeps at least `min_distinct_videos` videos when possible.
    """
    ...
    if apply_mmr:
        order=mmr(qv, doc_vecs, k=min(len(texts), max(12, final_k*2)), lambda_diversity=float(mmr_lambda))
    ...
    for ig,sc,tx,me in blended:
        ...
        if vid not in distinct and len(distinct)>=int(max(min_distinct_videos, 4)) and len(picked)>=int(final_k):
            continue
        ...
        if len(picked)>=int(final_k) and len(distinct)>=int(min_distinct_videos): break
    return picked
...
def _text_sim(embedder:SentenceTransformer, a:str,b:str)->float:
    va=embedder.encode([a],normalize_embeddings=True).astype("float32")[0]
    vb=embedder.encode([b],normalize_embeddings=True).astype("float32")[0]
...
def _first_bullets(vid:str, summaries:dict, k:int=2)->List[dict]:
    ...
    for b in summaries.get(vid,{}).get("bullets", []):
        ...
        if _quote_is_readable(q):
            out.append({"text":q, "ts":_format_ts(b.get("ts",0))})
            if len(out)>=k: break
    return out

def build_grouped_evidence_for_prompt(
    question:str, embedder:SentenceTransformer,
    hits:List[Dict[str,Any]], vm:dict, summaries:dict,
    routed_vids:List[str], max_quotes:int=3, min_sem_sim:float=0.15
)->Tuple[str,Dict[str,Any]]:
    """
    Guarantees ≥1 quote per routed video when possible.
    Filters quotes that are not semantically relevant to the question.
    """
    groups=group_hits_by_video(hits)

    # Preserve route order first, then any extras with hits
    order_vids = []
    seen=set()
    for v in routed_vids:
        if v not in seen and (v in groups or summaries.get(v)): order_vids.append(v); seen.add(v)
    ...
    for v_idx, vid in enumerate(order_vids, 1):
        info=vm.get(vid,{})
        if not info and vid not in groups:  # skip unknowns without hits
            continue
        ...
        # 1) validated, semantically relevant chunk quotes
        items = groups.get(vid, [])
        clean=[]
        for h in sorted(items, key=lambda r: float((r.get("meta") or {}).get("start",0))):
            q=_normalize_text(h.get("text",""))
            if not _quote_is_readable(q): continue
            if _text_sim(embedder, question, q) < min_sem_sim:  # semantic guard
                continue
            clean.append(h)
        ...
        # 3) nothing to show → skip listing to avoid confusing sources
        lines.append("")
...
def fetch_trusted_snippets(query:str, allowed_domains:List[str], max_snippets:int=3, per_domain:int=1, timeout:float=6.0):
    """
    Always attempts to return supporting evidence. If nothing is retrievable,
    returns homepages of vetted domains with short context.
    """
    ...
    for domain in allowed_domains:
        ...
        for url in links:
            try:
                ...
                # If page is very short, still keep link but mark as short.
                if len(text)<120:
                    out.append({"domain":domain,"url":url,"text":f"{domain} page loaded. Content too short for excerpt."})
                else:
...
def stream_openai_answer(model_name: str, question: str, history, grouped_video_block: str,
    ...
    """
    Generator that yields tokens for Streamlit to render incrementally.
    """
    ...
    user_payload = (("Recent conversation:\n"+"\n".join(convo)+"\n\n") if convo else "") + \
                   ...
                   "Write a detailed, well-grounded answer with explicit citations. Prefer multiple videos."
...
def _path_exists_report()->Dict[str, Any]:
    return {
        ...
        "domain_model": str(DOMAIN_MODEL_FILE), "domain_model_exists": DOMAIN_MODEL_FILE.exists(),
...
with st.sidebar:
    ...
    # Experts block directly under Auto Mode, auto expanded
    st.subheader("Experts")
    st.caption("Select the experts to include in routing and answers.")
    vm_sidebar = load_video_meta()
    vid_to_creator_sb, creator_to_vids_sb = build_creator_indexes_from_chunks(vm_sidebar)
    counts={canon: len(creator_to_vids_sb.get(canon,set())) for canon in ALLOWED_CREATORS}
    ...
    model_choice = st.selectbox(
        ...
        help="Use GPT-4o for best quality with streaming."
    )

    with st.expander("Advanced controls", expanded=False):
        st.caption("Defaults are tuned. Adjust only if needed.")
        st.number_input("Scan candidates first (K)", 256, 8000, 1024, 64, key="adv_scanK",
                        help="How many nearest chunks to fetch from FAISS before re-ranking.")
        st.number_input("Use top passages", 16, 160, 60, 2, key="adv_useK",
                        help="Final number of passages after MMR and caps.")
        st.number_input("Min distinct videos", 2, 12, 4, 1, key="adv_minvid",
                        help="Guarantee evidence spans at least this many videos when possible.")
        st.number_input("Passages per video cap", 1, 10, 4, 1, key="adv_cap",
                        help="Upper bound of quotes kept from a single video.")
        st.checkbox("Diversify with MMR", value=True, key="adv_mmr",
                    help="Reduces redundancy across selected passages.")
        st.slider("MMR balance", 0.1, 0.9, 0.45, 0.05, key="adv_lam",
                  help="Higher → prefer relevance; lower → prefer diversity.")
        st.slider("Recency weight", 0.0, 1.0, 0.20, 0.05, key="adv_rec",
                  help="Blend of recency into ranking.")
        st.slider("Recency half-life (days)", 7, 720, 270, 7, key="adv_hl",
                  help="How quickly recency decays.")
...
if _is_admin():
    ...
    st.caption(f"Unique users observed: **{_unique_user_count()}**")
    ...
    with cols_dbg[2]:
        ...
        if st.button("Rebuild summaries (fallback)"):
            # Lightweight fallback: pick salient lines per video
            try:
                if not CHUNKS_PATH.exists(): st.error("chunks.jsonl missing")
                else:
                    ...
                    summaries={}
                    vm_local = load_video_meta()
                    for vid in vids:
                        lines=texts_by_vid[vid][:800]
                        times=ts_by_vid[vid][:800]
                    ...
                    st.success(f"ok: wrote {VID_SUM_JSON}")
            except Exception as e:
                st.error(f"summaries fallback error: {e}")

    with st.expander("Creator inventory (from chunks.jsonl)", expanded=False):
        inv_vm = load_video_meta()
        inv = sorted(((c, len(vs)) for c,vs in build_creator_indexes_from_chunks(inv_vm)[1].items()),
                     key=lambda x: -x[1])
        ...
        for i, t in enumerate(st.session_state["turns"], 1):
            with st.expander(f"Turn {i}: {t.get('prompt','')[:80]}", expanded=False):
                ...
                if vids:
                    ...
                    vm_for_prev = load_video_meta()
                    cur_vid = None
                    for v in vids:
                        if v.get("video_id")!=cur_vid:
                            meta = vm_for_prev.get(v.get("video_id",""),{})
                            link = meta.get("url","")
...
# Stage A: route 8–12 videos
qv = embedder.encode([prompt], normalize_embeddings=True).astype("float32")[0]
routed_vids = route_videos_by_summary_and_domain(
    prompt, qv, summaries, vm, C, list(vid_list) if vid_list else list(universe),
    allowed_vids=allowed_vids_all,
    per_video_domain_probs=per_video_domain_probs, domain_top=domain_top,
    topK_min=8, topK_max=12,
    recency_weight=st.session_state.get("adv_rec", 0.20),
    half_life_days=st.session_state.get("adv_hl", 270),
    min_kw_overlap=1
)
candidate_vids = set(routed_vids) if routed_vids else allowed_vids_all

# Auto recall defaults
K_scan = st.session_state.get("adv_scanK", 1024)
K_use  = st.session_state.get("adv_useK", 60)
min_videos = st.session_state.get("adv_minvid", 4)
...
with st.spinner("Scanning selected videos…"):
    try:
        hits = stageB_search_chunks(
            ...
            final_k=int(K_use), min_distinct_videos=int(min_videos), per_video_cap=int(per_video_cap),
...
# Build evidence and stream the answer
grouped_block, export_struct = build_grouped_evidence_for_prompt(
    prompt, embedder, hits, vm, summaries, routed_vids=routed_vids, max_quotes=3, min_sem_sim=0.15
)
        ...
        if vids:
            ...
            for v in vids:
                if v.get("video_id")!=cur_vid:
                    ...
                    st.markdown(f"{hdr}" + (f"  •  [{link}]({link})" if link else ""))
                    cur_vid = v.get("video_id")
                # anchored link per quote if present
                if v.get("url"):
                    st.markdown(f"  • **{v.get('ts','')}** — “{_normalize_text(v.get('text',''))[:180]}”  ·  [{v.get('url')}]({v.get('url')})")
                else:
                    st.markdown(f"  • **{v.get('ts','')}** — “{_normalize_text(v.get('text',''))[:180]}”")
