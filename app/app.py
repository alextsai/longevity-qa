...
What changed
- Robust creator mapping so Healthy Immune Doc and Diary of a CEO never vanish due to label drift.
- Routing guardrails: centroid + keyword-overlap + recency; skip off-topic videos early.
- Recall that yields sensible quotes: softer filters, timestamp de-dup, ≥1 quote per routed video.
- Per-turn sources: each reply stores its own immutable sources; prior turns preserved.
- Web snippets work: dual DuckDuckGo modes + homepage fallback + on-screen trace for debugging.
- Centroid auto-renorm: enforce ||v||≈1.0 at load so similarity math is stable.
- Follow-ups: last 2 Q/A pairs inform routing and answer writing.
- Diagnostics/Admin: precompute freshness, rebuild, renorm, summaries-fallback, creator inventory.
- Comments: every major function documents assumptions and failure modes.

Environment
- Expects DATA_DIR with:
  data/
    chunks/chunks.jsonl
    index/faiss.index, metas.pkl, video_centroids.npy, video_ids.txt
    catalog/video_meta.json, video_summaries.json
"""
...
# Drop obviously bad combined labels
...
def _parse_ts(v)->float:
    """Parse HH:MM:SS or numeric seconds to float seconds."""
...
def _iso_to_epoch(iso:str)->float:
    """ISO8601 → epoch seconds, tolerate 'Z'."""
...
def _clear_chat():
    """Reset chat and per-turn sources."""
    st.session_state["messages"]=[]; st.session_state["turns"]=[]; st.rerun()


# ---------------- Admin gate ----------------
def _is_admin()->bool:
    """Enable admin with ?admin=1 and optional ?key=... if ADMIN_KEY present."""
...
def _raw_creator_of_vid(vid:str, vm:dict)->str:
    """Extract creator-ish field from video_meta.json; tolerate key drift."""
    info = vm.get(vid, {}) or {}
...
def _canonicalize_creator(name: str) -> str | None:
    """
    Robust canonicalization by tokens. Avoid brittle synonym tables.
...
def openai_answer(model_name: str, question: str, history, grouped_video_block: str,
                  web_snips: list[dict], no_video: bool, timeout_s: int = 55) -> str:
    """
    Compose answer from grouped video quotes + trusted sites.
    History carries last turns for follow-ups.
    """
    if not os.getenv("OPENAI_API_KEY"):
        return "⚠️ OPENAI_API_KEY is not set."

    # carry last 2 Q/A pairs
...
def _recency_score(published_ts:float, now:float, half_life_days:float)->float:
    """Exponential decay with half-life in days."""
...
def load_metas_and_model(index_path:Path=INDEX_PATH, metas_path:Path=METAS_PKL):
    ...
    Auto-renormalize video centroids if present and off-norm.
    """
    ...
    # Auto-renorm centroids to unit length once at load
    try:
        if VID_CENT_NPY.exists():
            ...
            if C.ndim==2 and C.size>0:
                ...
                if not np.allclose(n.mean(), 1.0, atol=0.1):
                    C = C / n
...
def load_video_centroids():
    ...
    # Ensure unit vectors for safety
    n = np.linalg.norm(C, axis=1, keepdims=True) + 1e-12
    C = C / n
    return C, vids
...
def _kw_score(text: str, query: str, idf: dict) -> Tuple[float,int]:
    ...
    The overlap count is an extra guard to avoid irrelevant videos.
    """
    ...
    overlap=len(qtok & set(ttok))
    score = sum(tf.get(w,0) * idf.get(w,0.0) for w in qtok) / (len(ttok)+1e-6)
...
def route_videos_by_summary(
    ...
    Guard: require at least `min_kw_overlap` keyword overlaps to reduce spurious picks.
    """
...
def stageB_search_chunks(query:str,
    ...
    Ensures quotes are substantial and relevant.
    """
    ...
    # Enforce per-video caps and total cap; keep quotes clean
    ...
    for ig,sc,tx,me in blended:
        ...
        if not _quote_is_valid(tx): continue
...
def build_grouped_evidence_for_prompt(hits:List[Dict[str,Any]], vm:dict, summaries:dict, max_quotes:int=3)->Tuple[str,Dict[str,Any]]:
    """
    Build human-readable block for the LLM **and** a structured export with per-quote timestamps.
    Guarantees ≥1 quote per video by falling back to raw chunks if summary bullets are thin.
    """
...
def _ddg_html(domain:str, query:str, headers:dict, timeout:float)->List[str]:
    """DuckDuckGo HTML endpoint; returns hrefs or []."""
    try:
...
def _ddg_lite(domain:str, query:str, headers:dict, timeout:float)->List[str]:
    """Lite endpoint sometimes works when HTML throttles."""
    try:
...
def fetch_trusted_snippets(query:str, allowed_domains:List[str], max_snippets:int=3, per_domain:int=1, timeout:float=6.0):
    """
    Try HTML → Lite → homepage fallback. Returns list of {domain,url,text}.
    Writes debug trace into st.session_state['web_trace'] for transparency.
    """
    trace=[]
    out=[]
...
def _build_summaries_fallback(max_lines_per_video: int = 800) -> str:
    ...
    Last-resort path if upstream summary job didn't run.
    """
    try:
        ...
        for i, t in enumerate(st.session_state["turns"], 1):
            with st.expander(f"Turn {i}: {t.get('prompt','')[:80]}"):
                ...
                if vids:
                    ...
                    # group by video to read better
                    by = collections.defaultdict(list)
                    for v in vids:
                        by[(v.get("video_id",""), v.get("title",""), v.get("creator",""))].append(v)
                    for (vid, title, creator), lst in by.items():
                        url = (load_video_meta().get(vid,{}) or {}).get("url","")
                        hdr = f"- **{title}** — _{creator}_"
                        st.markdown(f"{hdr}" + (f" — [link]({url})" if url else ""))
                        for q in lst:
...
with st.chat_message("assistant"):
    if not export_struct["videos"] and not web_snips:
        st.warning("No sufficiently relevant evidence after filtering.")
    ...
    with st.expander("Sources for this reply", expanded=True):
        vids = export_struct["videos"]; web = web_snips
        if vids:
            st.markdown("**Video quotes**")
            by = collections.defaultdict(list)
            for v in vids:
                by[(v.get("video_id",""), v.get("title",""), v.get("creator",""))].append(v)
            for (vid, title, creator), lst in by.items():
                url = (vm.get(vid,{}) or {}).get("url","")
                hdr = f"- **{title}** — _{creator}_"
                st.markdown(f"{hdr}" + (f" — [link]({url})" if url else ""))
                for q in lst:
                    st.markdown(f"  • {q.get('ts','')} — “{_normalize_text(q.get('text',''))}”")
        else:
        ...
        if web:
            ...
            for j, s in enumerate(web, 1):
                st.markdown(f"W{j}. [{s['domain']}]({s['url']})")
            tr=st.session_state.get("web_trace","")
            if tr: st.caption(f"web trace: {tr}")
        else:
            st.caption("No web snippets used.")

# Footer + Clear chat
st.caption("Auto routing selects top videos, diversified quotes are fused with supporting medical sites. Each reply keeps its own sources.")
cols=st.columns([1]*12)
with cols[-1]:
