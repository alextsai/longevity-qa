# -*- coding: utf-8 -*-
"""
Health | Nutrition Q&A — experts-first RAG + domain routing + trusted web (Phase 2, fix-set)

Fix set in this build
- UI: replace nested expanders with tabs to avoid StreamlitAPIException.
- Recall: widen search if Stage-B returns too few hits.
- Web: stricter filters + more high-quality medical domains + titles + better trace.
- Answering: enforce actionable protocol section; notify when no video evidence used.
- Admin: Content QA panel to spot missing bullets/urls quickly.
- Robustness kept: centroid renorm, offsets cache, turn snapshots, domain bootstrap.
- Embedder upgrade guard: use EMBEDDER_MODEL if FAISS dim matches, else fall back.

Infra assumptions
- DATA_DIR contains data/{chunks,index,catalog,domain}
...
TRUSTED_DOMAINS = [
    ...
    "aasm.org","sleepfoundation.org","who.int","nhs.uk",
    "familydoctor.org","healthfinder.gov","ama-assn.org",
    "medicalxpress.com","sciencedaily.com","nejm.org","med.stanford.edu","icahn.mssm.edu"
]
...
def stream_openai_answer(model_name: str, question: str, history, grouped_video_block: str,
    ...
    fallback_line = ("If no suitable video evidence exists, answer from trusted web snippets alone, "
                    ...
                    else "Trusted web snippets are supporting evidence.\n"
    ...
    system = (
        ...
        "Structure: Key summary • Practical protocol (numbered, 6–10 steps, each with mechanism and expected magnitude/time-to-effect if stated) • Safety notes.\n"
...
def build_grouped_evidence_for_prompt(
    ...
    for v_idx, vid in enumerate(order_vids, 1):
        info = vm.get(vid, {}) or {}
        if (vid not in groups) and not (summaries or {}).get(vid): continue
        ...
        lines.append(f"[Video {v_idx}] {title} — {creator}" + (f" — {date}" if date else "") + (f" — {base_url}" if base_url else ""))
        ...
        # fallback: summary bullets to guarantee ≥2 quotes
        need = max(0, 2 - used)
        if need > 0:
            for b in _first_bullets(vid, summaries or {}, k=need+1):
                ts_txt = b["ts"]; try_sec = 0.0
                if ts_txt:
        ...
        if used == 0 and items:
            h = items[0]; t0 = float((h.get("meta") or {}).get("start", 0))
            ts = _format_ts(t0); q = _normalize_text(h.get("text",""))
            lines.append(f"  • {ts}: “{q[:260]}”")
            export.append({"video_id": vid, "title": title, "creator": creator, "ts": ts, "text": q, "url": _yt_ts_url(base_url, t0)})
...
def fetch_trusted_snippets(query: str, allowed_domains: List[str],
                           max_snippets: int = 4, per_domain: int = 2, timeout: float = 6.0):
    trace, out = [], []
    ...
    for domain in allowed_domains:
        ...
        for url in links[:per_domain]:
            try:
                if re.search(r"/(signin|login|subscribe|account)\b", url or "", re.I):
                    continue
                ...
                # filters: discard thin or off-topic pages
                if len(text) < 300:
                    trace.append(f"{domain}: {url} too short")
                    continue
                if url.rstrip("/").split("/")[-1] in {"", "index.html"} and len(text) < 600:
                    trace.append(f"{domain}: homepage too generic")
                    continue
                if _kw_overlap(query, text) < 3:
                    trace.append(f"{domain}: low keyword overlap")
                    continue
...
def _record_session_hit():
    try:
        ...
        now = int(time.time())
        if sid in d:
            d[sid]["last"] = now; d[sid]["hits"] = d[sid].get("hits", 0) + 1
        else:
...
if _is_admin():
    ...
    with st.expander("Content QA"):
        sums = load_video_summaries() or {}
        vm_  = load_video_meta() or {}
        missing_bullets = [vid for vid,v in sums.items() if not v.get("bullets")]
        missing_urls    = [vid for vid,v in vm_.items() if not v.get("url")]
        st.write({"videos": len(vm_), "summaries": len(sums),
                  "missing_bullets": len(missing_bullets), "missing_urls": len(missing_urls)})
        if missing_bullets:
            st.caption("Examples missing bullets:")
            st.code(missing_bullets[:10])
    ...
    if st.session_state["turns"]:
        ...
        for i, t in enumerate(st.session_state["turns"], 1):
            with st.expander(f"Turn {i}: {t.get('prompt','')[:80]}", expanded=False):
                ...
                vids = t.get("videos", []); web = t.get("web", [])
                tabs = st.tabs(["Video quotes", "Trusted websites", "Web fetch trace"])
                with tabs[0]:
                    if vids:
                        cur_vid = None; vm_prev = load_video_meta()
                        for v in vids:
                ...
                with tabs[1]:
                    if web:
                        ...
                        st.caption("No trusted web snippets used.")
                with tabs[2]:
                    st.code(t.get("web_trace","no trace"))
...
# Selected experts → allowed universe
vid_to_creator, creator_to_vids = build_creator_indexes_from_chunks(vm)
...
# Widen recall if thin
if len(hits) < max(6, int(0.25 * K_use)):
    hits = stageB_search_chunks(
        prompt, index, embedder, allowed_vids_all,
        initial_k=min(int(K_scan), index.ntotal if index is not None else int(K_scan)),
        final_k=int(K_use), max_videos=int(max_videos), per_video_cap=int(per_video_cap),
        apply_mmr=bool(use_mmr), mmr_lambda=float(mmr_lambda),
        recency_weight=float(recency_weight), half_life_days=float(half_life), vm=vm
...
if allow_web and requests and BeautifulSoup and int(max_web_auto) > 0:
    with st.spinner("Fetching trusted websites…"):
        ...
        web_snips = fetch_trusted_snippets(prompt, domains, max_snippets=int(max_web_auto), per_domain=2)
        if len(web_snips) < 2:
            more = fetch_trusted_snippets(prompt, TRUSTED_DOMAINS, max_snippets=2, per_domain=2)
...
with st.chat_message("assistant"):
    if not export_struct["videos"]:
        st.info("No video passages passed relevance gates. Answer will rely on trusted web.")
    ...
    with st.expander("Sources for this reply", expanded=False):
        ...
        with tabs[2]:
            st.code(st.session_state.get("web_trace","no trace"))
