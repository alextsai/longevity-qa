# -*- coding: utf-8 -*-
"""
Health | Nutrition Q&A — experts-first RAG + domain routing + trusted web, with streaming.
- Accurate per-video quotes with stricter validation and timestamped links
- Domain router (uses data/domain/domain_model.joblib + domain_probs.* if present)
- Trusted medical sites as supporting evidence with visible links
- Streaming answers; sources auto-collapsed; admin diagnostics restored
"""
...
# Mounted volume defaults to /var/data; our artifacts live under /var/data/data/*
...
# ---------- Bootstrap domain artifacts from repo into volume if missing ----------
...
def verify_domain_artifacts()->dict:
    ...
    try:
        mp = DOMAIN_MODEL if DOMAIN_MODEL.exists() else repo/"domain_model.joblib"
        if joblib and mp.exists():
            pipe = joblib.load(mp)
...
TRUSTED_DOMAINS = [
    ...
    "familydoctor.org","healthfinder.gov","ama-assn.org","nejm.org","med.stanford.edu",
]
...
def route_videos_by_summary(query: str, qv: np.ndarray, summaries: dict, vm: dict,
                            ...
                            topK:int, recency_weight:float, half_life_days:float, min_kw_overlap:int=2)->list[str]:
    ...
    for v in universe:
        ...
        rec = 0.0
        info=(vm or {}).get(v,{})
        rec = 0.5 ** (max(0.0,(now-_iso_to_epoch(info.get("published_at") or info.get("publishedAt") or info.get("date") or ""))/86400.0) / max(1e-6, half_life_days))
        base = 0.6*cs + 0.3*kw
...
# ---------- Stage B: dense search + caps ----------
def stageB_search_chunks(query:str, index:faiss.Index, embedder:SentenceTransformer,
                         candidate_vids:Set[str]|None, initial_k:int, final_k:int,
                         max_videos:int, per_video_cap:int, apply_mmr:bool, mmr_lambda:float,
...
def build_grouped_evidence_for_prompt(hits:List[Dict[str,Any]], vm:dict, summaries:dict,
                                      routed_vids:List[str], max_quotes:int=3)->Tuple[str,Dict[str,Any]]:
...
def fetch_trusted_snippets(query:str, allowed_domains:List[str], max_snippets:int=3, per_domain:int=1, timeout:float=6.0):
    ...
    if not requests or not BeautifulSoup or max_snippets<=0:
        st.session_state["web_trace"]="requests/bs4 unavailable"; return []
    ...
    for domain in allowed_domains:
        ...
        if not links:
            links=[f"https://{domain}"]; trace.append(f"{domain}: homepage fallback")
        ...
        for url in links:
            try:
                ...
                if r.status_code!=200:
                    trace.append(f"{domain}: {url} status {r.status_code}"); continue
                soup=BeautifulSoup(r.text,"html.parser")
                paras=[p.get_text(" ",strip=True) for p in soup.find_all("p")]
                text=_normalize_text(" ".join(paras))[:1800]
                if len(text)<200: trace.append(f"{domain}: {url} too short"); continue
...
def stream_answer(model_name: str, question: str, history, grouped_video_block: str,
    ...
    system = (
        ...
        + fallback_line +
        "Rules:\n"
        "• Cite every claim: (Video k) per video, (Wj DOMAIN) for web.\n"
        "• Distinguish human clinical vs mechanistic.\n"
        "• Normalize units; include numeric effects if present.\n"
        "• Provide practical protocol + safety notes; no diagnosis.\n"
...
with st.sidebar:
    ...
    # Experts directly under Auto Mode, auto-expanded by listing checkboxes here
    vm = load_video_meta()
...
# Admin diagnostics (restored)
if show_diag and _is_admin():
    ...
    with c2:
        if st.button("Rebuild precompute (admin)"):
            try:
                ...
                spec = importlib.util.spec_from_file_location("pvs", str(p))
                mod = importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)  # type: ignore
                mod.main(); st.success("ok: rebuilt summaries/centroids")
            except Exception as e:
                st.error(f"precompute error: {e}")
    with c3:
        if st.button("Renormalize centroids"):
            try:
                C = np.load(VID_CENT_NPY).astype("float32")
                C = C / (np.linalg.norm(C,axis=1,keepdims=True)+1e-12)
                np.save(VID_CENT_NPY, C); st.success("ok: saved")
            except Exception as e:
...
if prompt is None:
    if st.session_state["turns"]:
        ...
        # Each turn collapsible; avoid nesting expanders inside expanders
        for i, t in enumerate(st.session_state["turns"], 1):
            exp = st.expander(f"Turn {i}: {t.get('prompt','')[:80]}", expanded=False)
            with exp:
                ...
                if web:
                    ...
                    if t.get("web_trace"):
                        st.caption(f"web fetch: {t['web_trace']}")
...
if domain_top and per_video_domain_probs:
    ...
    scored.sort(key=lambda x:-x[1])
    slice_set = set([vid for vid,sc in scored if sc>0][:200]) or allowed_vids_all
...
with st.chat_message("assistant"):
    ...
    this_turn = {"prompt": prompt, "answer": ans, "videos": export_struct["videos"], "web": web_snips,
                 "web_trace": st.session_state.get("web_trace","")}
    st.session_state["turns"].append(this_turn)

# Sources UI for THIS reply — auto-collapsed; no nested expanders
st.markdown("---")
src_exp = st.expander("Sources for this reply", expanded=False)
with src_exp:
    vids = st.session_state["turns"][-1]["videos"]
    web  = st.session_state["turns"][-1]["web"]
    if vids:
        st.markdown("**Video quotes**")
        cur_vid=None
        for v in vids:
    ...
    if web:
        ...
        if st.session_state.get("web_trace"):
            st.caption(f"web fetch: {st.session_state['web_trace']}")
