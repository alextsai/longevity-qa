...
- Accurate routing: hybrid score = (video centroid • query) + keyword overlap + recency decay.
- Relevant recall: diversify with MMR, soft quote filter, require keyword overlap, ensure ≥1 quote/video.
- Per-turn sources: each reply stores its own sources; previous replies remain intact and viewable.
- Trusted web: DuckDuckGo HTML→Lite→homepage fallback; show trace; 1 snippet/domain, up to 3 total.
- Healthy Immune Doc + Diary mapping: heuristic canonicalizer fixes label variance across dumps.
- Self-check: after each answer, display a compact diagnostic of quote relevance and coverage.
- Admin: one-click rebuild, centroid re-norm (also auto on load), summaries fallback, creator inventory.

Assumptions
- DATA_DIR has: data/chunks/chunks.jsonl, data/index/faiss.index, data/index/metas.pkl,
  data/catalog/video_meta.json, and optionally precompute outputs.
...
# Precompute outputs (optional but recommended)
...
# Displayed names we consider "experts-first"
ALLOWED_CREATORS = [
...
# ---------- Video meta + creator canonicalization ----------
...
def _canonicalize_creator(name: str) -> str | None:
    """
    Heuristic mapping so Healthy Immune Doc and Diary variants always resolve.
    This avoids brittle synonym tables and absorbs noisy dumps.
    """
...
def load_video_centroids():
    """Also auto-renormalizes to unit length to prevent drift."""
    ...
    try:
        if status["centroids_present"] and status["ids_present"]:
            ...
            if status["chunks_mtime"] > max(status["cent_mtime"], status["ids_mtime"]):
                status["msg"].append("chunks.jsonl newer than centroids/ids → rebuild.")
        else:
            status["msg"].append("Centroids/IDs missing → rebuild.")
...
# ---------- Trusted web fetch with visible trace ----------
...
def fetch_trusted_snippets(query:str, allowed_domains:List[str], max_snippets:int=3, per_domain:int=1, timeout:float=6.0):
    ...
    for domain in allowed_domains:
        ...
        mode="html"
        if not links:
            links=_ddg_lite(domain, query, headers, timeout); mode="lite"
        if not links:
            links=[f"https://{domain}"]; mode="fallback"
        trace.append(f"{domain}:{mode}:{len(links)}")
        ...
        for url in links:
            try:
                ...
                if len(text)<200:
                    trace.append(f"{domain}:{url}:short")
                    continue
                out.append({"domain":domain,"url":url,"text":text})
            except Exception as e:
                trace.append(f"{domain}:{url}:err:{e}")
...
def openai_answer(model_name: str, question: str, history, grouped_video_block: str,
    ...
    system = (
        ...
        "Structure: Key summary • Practical protocol • Safety notes. Use only provided quotes/snippets."
    )
...
with st.sidebar:
    ...
    st.caption("Adds short vetted excerpts as secondary evidence.")
    ...
    with st.expander("Advanced (technical controls)", expanded=False):
        st.caption("Defaults are tuned for most queries.")
...
if _is_admin():
    ...
    if not (ok_cent and ok_ids):
        st.warning("Centroids/IDs older than chunks.jsonl. Use rebuild or fallback.")
            ...
            with st.expander(f"Turn {i}: {t.get('prompt','')[:80]}"):
                ...
                if vids:
                    ...
                    for v in vids:
                        ...
                        url = info.get("url","")
                        header = f"- **{v.get('title','')}** — _{v.get('creator','')}_"
                        st.markdown(f"{header} • {v.get('ts','')}" + (f" • [link]({url})" if url else ""))
...
# Routing query includes last user message for follow-ups
...
# Stage B search over routed videos
...
with st.chat_message("assistant"):
    if not export_struct["videos"] and not web_snips:
        st.warning("No relevant expert video evidence found.")
    ...
    # Sources for THIS reply (not previous)
    with st.expander("Sources for this reply", expanded=True):
        vids = export_struct["videos"]
        if vids:
            st.markdown("**Video quotes**")
            # Group by video_id to show link once
            by_vid = {}
            for v in vids:
                by_vid.setdefault(v["video_id"], {"meta": v, "quotes": []})
                by_vid[v["video_id"]]["quotes"].append(v)
            for vid, pack in by_vid.items():
                meta = pack["meta"]
                info = vm.get(vid, {}) or {}
                url = info.get("url","")
                header = f"- **{meta.get('title','')}** — _{meta.get('creator','')}_"
                st.markdown(f"{header}" + (f" • [link]({url})" if url else ""))
                for q in pack["quotes"]:
                    st.markdown(f"  • **{q.get('ts','')}** — “{_normalize_text(q.get('text',''))[:160]}”")
        else:
        ...
        if st.session_state.get("web_trace"):
            st.caption(f"web trace: {st.session_state['web_trace']}")

    # Self-check: show quick relevance diagnostics
    with st.expander("Self-check (diagnostic)", expanded=False):
        # Count experts and quotes
        experts = list({v.get("creator","") for v in export_struct["videos"]})
        st.markdown(f"- Videos used: **{len(set(v.get('video_id') for v in export_struct['videos']))}**")
        st.markdown(f"- Quotes used: **{len(export_struct['videos'])}**")
        st.markdown(f"- Experts covered: **{len(experts)}** — {', '.join(experts)}")
        if routed_vids:
            labs=[_canonicalize_creator(_raw_creator_of_vid(v, vm)) or _raw_creator_of_vid(v, vm) for v in routed_vids]
            st.markdown(f"- Routed videos: {len(routed_vids)} — experts: {', '.join(sorted(set(labs)))}")
        if allow_web and selected_domains:
            st.markdown(f"- Web selected: {len(web_snips)} of max {max_web_auto}")

# Footer
st.caption("Each reply keeps its own sources. Use Clear chat to start a new thread.")
cols=st.columns([1]*12)
with cols[-1]:
