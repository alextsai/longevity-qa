...
# ---------------- Safe, low-memory runtime knobs (set before imports) ----------------
import os
...
try:
    ...
    torch = None  # guard for environments without torch
...
from utils.labels import label_and_url  # local helper for pretty labels/URLs
...
def mmr_rerank_lite(
    ...
    """
    Hard-capped pool to protect memory. All embeddings are L2-normalized.
    """
    pool = min(len(hits), max(int(final_k) * 3, 60), 100)  # <=100 candidates
    cand_embs = cand_embs[:pool]
    hits = hits[:pool]
    if pool == 0:
        return []

    sim_q = cand_embs @ q_emb  # (pool,)
    sims = cand_embs @ cand_embs.T  # (pool, pool)
...
def search_chunks(
    ...
    with ctx:
        q = embedder.encode([query], normalize_embeddings=True).astype("float32")[0]  # (d,)
    ...
    pool = min(len(raw), max(int(final_k) * 3, 60), 100)
...
def openai_answer(model_name: str, question: str, history, hits) -> str:
    ...
    # keep at most 3 prior turns to minimize tokens
    ctx_msgs = []
    ...
    # numbered, compact evidence; clamp each excerpt
    ev = []
...
with st.sidebar:
    ...
    initial_k = st.number_input(
        ...
        min_value=20, max_value=2000, value=96, step=16,
    ...
    final_k = st.number_input(
        ...
        help="Chunks sent to the model as evidence. Balance recall vs cost and latency."
    )
    ...
    # Primary sources by videos (not chunks), link out
    vm = load_video_meta()
    ...
    with st.expander("Health checks", expanded=False):
        ...
        st.caption("Expect 'pong'. If ERR or NO_KEY appears, fix OPENAI_API_KEY or network egress.")
...
if index is None or payload is None or not texts:
    with st.chat_message("assistant"):
        st.error("FAISS index / metas / chunks not found or empty.")
    st.stop()

# Alignment checks to avoid silent failures
if int(index.ntotal) != len(texts):
    with st.chat_message("assistant"):
        st.error(
            f"Index size mismatch. FAISS ntotal={int(index.ntotal):,} but chunks.jsonl has {len(texts):,} rows. "
            "Rebuild the index with this chunks file."
        )
    st.stop()

if metas_from_pkl is not None and len(metas_from_pkl) != len(texts):
    with st.chat_message("assistant"):
        st.error("metas.pkl length does not match chunks.jsonl. Rebuild artifacts together.")
    st.stop()
