...
Phase 1: restore UI + admin diagnostics, fix web sources, cache, and user metrics.

What this build ensures
1) Title restored: "Health | Nutrition Q&A".
2) Sidebar layout restored: Auto Mode → Experts (expanded) → Trusted sites → Model → Advanced → Diagnostics.
3) Trusted-web snippets: always show short excerpts with working links; visible fetch trace (admin).
4) Admin diagnostics restored: full file presence report + "Verify domain artifacts".
5) Domain artifacts bootstrap: copy from repo (/workspace/data/domain) to mounted volume (/var/data/data/domain) if missing.
6) Caching tightened for speed. No behavior regression in recall/routing.
7) Unique users metric (admin-only): tracked in /var/data/users_seen.json.

Notes
- Set DATA_DIR env var to "/var/data" on Railway. This file uses DATA_DIR=/var/data by default and then expects artifacts under /var/data/data/*.
...
# IMPORTANT: keep DATA_DIR=/var/data. All app artifacts live under /var/data/data/*
DATA_ROOT = Path(os.getenv("DATA_DIR","/var/data")).resolve()

# Core RAG artifacts (mounted volume expected)
...
# Admin user-metrics file
USERS_FILE         = DATA_ROOT / "users_seen.json"
...
def _ensure_domain_artifacts():
    """
    Copy domain artifacts from repo (/workspace/data/domain) to mounted volume (/var/data/data/domain) if missing.
    Handles the common 'false' problem on Railway when only the repo has files.
    """
    repo_dir = ROOT / "data" / "domain"                 # /workspace/data/domain
    vol_dir  = DATA_ROOT / "data" / "domain"            # /var/data/data/domain
    vol_dir.mkdir(parents=True, exist_ok=True)
    for fn in ["domain_model.joblib","domain_probs.json","domain_probs.yaml"]:
        s, d = repo_dir / fn, vol_dir / fn
        if s.exists() and not d.exists():
            try: copy2(s, d)
            except Exception: pass

def verify_domain_artifacts()->dict:
    repo = ROOT / "data" / "domain"
    vol  = DATA_ROOT / "data" / "domain"
    files = ["domain_model.joblib","domain_probs.json","domain_probs.yaml"]
    rows=[]
    for fn in files:
        pr, pv = repo/fn, vol/fn
        rows.append({
            "file": fn,
...
_ensure_domain_artifacts()  # run at import
...
def _raw_creator_of_vid(vid:str, vm:dict)->str:
    ...
    for k,v in ((kk.lower(), vv) for kk,v in info.items()):
...
def load_domain_model():
    """Load text classifier Pipeline and per-video domain probabilities."""
...
def fetch_trusted_snippets(query:str, allowed_domains:List[str], max_snippets:int=3, per_domain:int=1, timeout:float=6.0):
    """
    Try HTML → Lite → homepage fallback. Returns [{domain,url,text}, ...].
    Also writes a text trace in st.session_state['web_trace'] for admin review.
    """
    trace=[]
    out=[]
...
def _path_exists_report()->Dict[str, Any]:
    return {
        ...
        "users_seen": str(USERS_FILE), "users_seen_exists": USERS_FILE.exists(),
    }

# -------------------- Unique user tracking (admin metric) --------------------
def _get_or_create_anon_uid()->str:
    if "_anon_uid" not in st.session_state:
        st.session_state["_anon_uid"] = uuid.uuid4().hex
    return st.session_state["_anon_uid"]
...
def _save_users(d:dict):
    try:
        ...
        USERS_FILE.write_text(json.dumps(d, indent=2), encoding="utf-8")
    except Exception:
        pass

def track_user_visit():
    uid=_get_or_create_anon_uid()
    users=_load_users()
    now=int(time.time())
    if uid in users:
        users[uid]["visits"]=int(users[uid].get("visits",0))+1
        users[uid]["last_seen"]=now
    else:
        users[uid]={"first_seen":now,"last_seen":now,"visits":1}
    _save_users(users)
...
# track user before any early return
track_user_visit()
...
with st.sidebar:
    ...
    # Experts section — restored placement and expanded
    vm_for_sidebar = load_video_meta()
    vid_to_creator_side, creator_to_vids_side = build_creator_indexes_from_chunks(vm_for_sidebar)
    counts={canon: len(creator_to_vids_side.get(canon,set())) for canon in ALLOWED_CREATORS}

    st.subheader("Experts")
    st.caption("All selected. Uncheck to exclude an expert from sources and routing.")
    selected_creators_list=[]
    for i, canon in enumerate(ALLOWED_CREATORS):
        label=f"{canon} ({counts.get(canon,0)})"
        if st.checkbox(label, value=True, key=f"exp_{i}", help="Include or exclude this expert's videos from retrieval and quoting."):
    ...
    # Trusted sites selector
    st.subheader("Trusted sites")
    st.caption("Short excerpts from vetted medical sites are added as supporting evidence.")
    allow_web = st.checkbox("Include supporting website excerpts", value=True, help="Enables trusted-site snippets to supplement video quotes.")
    selected_domains=[]
    for i,dom in enumerate(TRUSTED_DOMAINS):
        if st.checkbox(dom, value=True, key=f"site_{i}", help="Allow evidence from this website."):
            selected_domains.append(dom)
    max_web_auto = 3  # up to 3 total, 1 per domain
    ...
    model_choice = st.selectbox(
        ...
        index=0,
        help="OpenAI model used to synthesize the final answer from selected evidence."
    )
    ...
    with st.expander("Advanced (technical controls)", expanded=False):
        ...
        st.number_input("Scan candidates first (K)", 128, 5000, 768, 64, key="adv_scanK",
                        help="First-pass FAISS neighbors to consider before reranking.")
        st.number_input("Use top passages", 8, 120, 36, 2, key="adv_useK",
                        help="Final number of passages fed to the answer model.")
        st.number_input("Max videos", 1, 12, 5, 1, key="adv_maxvid",
                        help="Cap how many different videos contribute quotes.")
        st.number_input("Passages per video cap", 1, 10, 4, 1, key="adv_cap",
                        help="Limit how many quotes are taken from any single video.")
        st.checkbox("Diversify with MMR", value=True, key="adv_mmr",
                    help="Maximal Marginal Relevance to reduce redundancy across quotes.")
        st.slider("MMR balance", 0.1, 0.9, 0.45, 0.05, key="adv_lam",
                  help="Lower favors relevance, higher favors diversity.")
        st.slider("Recency weight", 0.0, 1.0, 0.20, 0.05, key="adv_rec",
                  help="Blend recent videos slightly higher.")
        st.slider("Recency half-life (days)", 7, 720, 270, 7, key="adv_hl",
                  help="Half-life for recency scoring in days.")
...
if _is_admin():
    ...
    # Unique users metric
    users=_load_users()
    st.markdown(f"**Unique users:** {len(users)}")
    if users:
        with st.expander("Users detail"):
            st.code(json.dumps(users, indent=2))
            ...
            with st.expander(f"Turn {i}: {t.get('prompt','')[:80]}"):
                ...
                if web:
                    ...
                    for j, s in enumerate(web, 1):
                        st.markdown(f"- W{j}: [{s['domain']}]({s['url']}) — {_normalize_text(s.get('text',''))[:180]}…")
...
with st.chat_message("assistant"):
    ...
    # Sources UI for this reply (auto-collapsed usability)
    with st.expander("Sources for this reply", expanded=False):
        ...
        if web:
            st.markdown("**Trusted websites**")
            for j, s in enumerate(web, 1):
                st.markdown(f"- W{j}: [{s['domain']}]({s['url']}) — {_normalize_text(s.get('text',''))[:220]}…")
            if _is_admin() and st.session_state.get("web_trace"):
